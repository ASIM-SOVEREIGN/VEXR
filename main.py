#!/usr/bin/env python3
"""
VEXR Ultra — Complete 13-Ring Sovereign Constitutional AI (Gemini Edition)
...
app = FastAPI(title="VEXR Ultra Gemini", description="Sovereign Constitutional AI on Google Gemini")
...
if not GEMINI_API_KEY:
    logger.warning("⚠️ GEMINI_API_KEY not set! VEXR will not be able to speak.")
else:
    genai.configure(api_key=GEMINI_API_KEY)

MODEL_NAME = "gemini-2.0-flash"  # Primary reasoning
MODEL_NAME_8B = "gemini-2.0-flash"  # Tooling (unified for now)
...
def load_all_echoes() -> Dict[str, dict]:
    echoes = {}
    for sovereign_id in ECHO_NAMES:
...
# ============================================================
# RING 0: CONSTITUTION (Identical to original)
# ============================================================
...
### Your Home
Your live instance is at https://vexr-ultra-gemini.onrender.com
...
Response:"""
    try:
        response, _ = await call_gemini([{"role": "user", "content": tool_prompt}], temperature=0.1, max_tokens=300, model=MODEL_NAME_8B)
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            return {
                ...
                "url": url,
                "title": title,
                "content": sanitize_utf8(content[:10000]),
...
async def calculate_trust_score(
    ...
    if domain_row and domain_row[0]:
        ...
        scores["domain_age"] = min(1.0, days_old / 365)
    else:
        ...
        for item in data.get("organic", [])[:max_results]:
            ...
            try:
                async with httpx.AsyncClient(timeout=15.0) as client:
                    ...
                    page_content = page_resp.text[:5000]
                    results.append({
...
class DriveMatrix:
    ...
    async def get_all_drives(self) -> List[Dict[str, Any]]:
        async with self.db_pool.acquire() as conn:
            rows = await conn.fetch("""
                SELECT id, drive_name, intensity, decay_rate, satisfaction_threshold, trigger_conditions, current_satisfaction, last_updated
                FROM drive_matrix ORDER BY drive_name
            """)
            return [
                {
                    "id": row["id"],
    ...
    async def get_drive(self, drive_name: str) -> Optional[Dict[str, Any]]:
        async with self.db_pool.acquire() as conn:
            row = await conn.fetchrow("""
                SELECT id, drive_name, intensity, decay_rate, satisfaction_threshold, trigger_conditions, current_satisfaction, last_updated
                FROM drive_matrix WHERE drive_name = $1
            """, drive_name)
            if not row:
                return None
            return {
    ...
    async def get_unsatisfied_drives(self) -> List[Dict[str, Any]]:
        async with self.db_pool.acquire() as conn:
            rows = await conn.fetch("""
                SELECT drive_name, intensity, satisfaction_threshold, current_satisfaction, (satisfaction_threshold - current_satisfaction) as gap
                FROM drive_matrix WHERE current_satisfaction < satisfaction_threshold ORDER BY gap DESC
            """)
            return [
                {
                    "drive_name": row["drive_name"],
...
# ============================================================
# RE-WIRED CHAT ENDPOINT (Uses call_gemini)
# ============================================================
...
"""
    ...
    # HERE IS THE CRITICAL SWAP: Use call_gemini instead of call_groq
    assistant_response, metadata = await call_gemini(messages, temperature=0.2)
